...
template<typename T, size_t Size, size_t Index = 0>
void array_to_string(const Array<T, Size> array, StringBuilder& builder)
{
    ...
    if constexpr (Index < Size) {
        builder.appendff(" {}", array[Index]);
        array_to_string<T, Size, Index + 1>(array, builder);
    }
}
...
struct Formatter<Array<T, Size>> : Formatter<StringView> {
    void format(FormatBuilder& builder, const Array<T, Size>& array)
    {
        StringBuilder string_builder;
        AK::Detail::array_to_string(array, string_builder);
        Formatter<StringView>::format(builder, string_builder.to_string());
    }
};
