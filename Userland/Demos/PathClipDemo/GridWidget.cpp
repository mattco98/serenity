...
void InputGridWidget::mousedown_event(GUI::MouseEvent& event)
{
    ...
    for (auto& line : m_primary_path.split_lines()) {
        if (dragged_point == line.from || dragged_point == line.to) {
            m_point_being_dragged = dragged_point.to_type<float>();
...
void InputGridWidget::mousemove_event(GUI::MouseEvent& event)
{
    ...
    m_point_being_dragged = current_point.to_type<float>();
...
Gfx::IntPoint InputGridWidget::get_closest_grid_point_to(Gfx::IntPoint const& point) const
{
    return {
...
void OutputGridWidget::set_clip_type(Gfx::PathClipping::ClipType type)
{
    m_clip_type = type;
...
struct Formatter<Vector<T>> : Formatter<StringView> {
    ErrorOr<void> format(FormatBuilder& format_builder, Vector<T> const& vec)
    {
        ...
        for (auto const& el : vec)
            builder.appendff("  {}\n", el);
        builder.append(']');
        TRY(Formatter<StringView>::format(format_builder, builder.to_string()));
        return {};
    }
};
