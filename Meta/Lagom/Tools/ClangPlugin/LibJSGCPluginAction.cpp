...
void LibJSGCVisitor::validate_record_macros(clang::CXXRecordDecl const& record)
{
    ...
    auto report_missing_macro = [&] {
        auto diag_id = diag_engine.getCustomDiagID(clang::DiagnosticsEngine::Warning, "Expected record to have a %0 macro invocation");
        auto builder = diag_engine.Report(record.getLocation(), diag_id);
    ...
    for (auto const& macro : it->second) {
        if (record_range.fullyContains(macro.range)) {
            ...
            if (found_macro) {
                auto diag_id = diag_engine.getCustomDiagID(clang::DiagnosticsEngine::Warning, "Record has multiple JS_CELL-like macro invocations");
                diag_engine.Report(record_range.getBegin(), diag_id);
            }

            found_macro = true;
            if (macro.type != expected_cell_macro_type) {
                auto diag_id = diag_engine.getCustomDiagID(clang::DiagnosticsEngine::Warning, "Invalid JS-CELL-like macro invocation; expected %0");
            ...
            if (macro.args[0].text != record_name) {
                auto diag_id = diag_engine.getCustomDiagID(clang::DiagnosticsEngine::Warning, "Expected first argument of %0 macro invocation to be %1");
            ...
            if (expected_cell_macro_type == LibJSCellMacro::Type::JSPrototypeObject) {
                ...
                auto diag_id = diag_engine.getCustomDiagID(clang::DiagnosticsEngine::Warning, "Expected second argument of %0 macro invocation to be %1");
